...
class Bluerails_Settings {
	...
	public function render_section_intro() {
		...
		echo '<p>' . esc_html__( 'Every reported hit also includes the raw Referer header, and on a request whose User-Agent matches no known bot signature, the plugin additionally checks that header against a small allow-list of AI-assistant domains as a fallback signal.', 'bluerails-agent-traffic' ) . '</p>';
	}
	...
	public function render_behavioral_section_intro() {
		echo '<p>' . esc_html__( 'BLUE-1474: an additional signal, enabled by default as of version 1.3.0, that runs a small JS snippet in visitors\' browsers to help identify rendered-browser AI agents (browser-extension agentic sessions such as ChatGPT\'s Chrome extension/Work app or Anthropic\'s Claude for Chrome) that present as an ordinary Chrome browser with no distinguishing User-Agent or Referer. Uncheck the box below if you would rather not run it on this site.', 'bluerails-agent-traffic' ) . '</p>';
